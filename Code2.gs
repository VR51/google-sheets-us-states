...
var by_full_c = {};
for (var prop in by_abbr_c) {
    if(by_abbr_c.hasOwnProperty(prop)) {
        by_full_c[by_abbr_c[prop].toUpperCase()] = prop;
    }
}

/**
 * Convert a Country ISO Code abbreviation to its full name.
 *
...
function COUNTRY_ABBREVIATION_TO_FULL(input) {
  if (input.map) {            // Test whether input is an array.
    ...
    return by_abbr_c[input.toUpperCase()];
  }
}

/**
 * Convert a Country name to its ISO Code abbreviation.
 *
 * @param {text} input The value or range of cells to convert.
...
function COUNTRY_FULL_TO_ABBREVIATION(input) {
  if (input.map) {            // Test whether input is an array.
    ...
    return by_full_c[input.toUpperCase()];
  }
}
